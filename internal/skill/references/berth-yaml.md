...
| --- | --- |
| `version` | Must be `1`. |
| `base` | Baseline branch for new workspaces; defaults to `main`. When it names a configured remote (for example `origin/main`) whose remote-tracking ref is absent, berth fetches that ref first. |
...
| `processes.<name>.shutdown` | Passed to process-compose (`command`, `signal`, `timeout_seconds`, `parent_only`). When present it is authoritative, and berth does not add its default shutdown bound. |
...
- **The contract is immutable in place.** Changing `runtime.*` or the named-port
  set makes the existing workspace unusable for `berth up` ("port contract
  changed; create a new workspace"). The registration is never stranded: reclaim
  it with `berth done` (or `berth gc`) and create a workspace from the updated
  branch.
