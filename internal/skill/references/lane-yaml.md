...
Read this when writing or debugging a `lane.yaml`. The file lives at the
...
| `worktree_root` | Where worktrees are created. Relative values resolve inside the repository, absolute values do not; the default is the sibling `<repo>.lanes` directory. |
...
| `ports` | Named ports such as `[web, pg]`. Port names become `LANE_PORT_<NAME>`; the name `pc` is reserved. |
...
- **Reserved names.** `pc` cannot be a port name, and `env` keys cannot start with
  `LANE_`, `GIT_DIR` or `GIT_WORK_TREE` — lane injects those into every process.
- **Ports in probes.** A probe runs in the execution context, so a container
  probe uses the internal `listen` port (`8080`), never `LANE_HOST_PORT_*`.
- **`copy_dirs` and `.worktreeinclude` differ.** `copy_dirs` copies whole
  ...
  run during setup: `lane new`, `lane reset`, `lane adopt --setup`.
- **`.lane/` is machine state.** It holds the private data directory, generated
...
in the lane repository is the reference, and takes `BASE_IMAGE` as a build arg.
...
workspace is mounted at `/workspace` and `$LANE_DATA_DIR` is
`/workspace/.lane/data`. Publications are TCP over IPv4 loopback, and
